      ...
      <div className="flex flex-col justify-center items-center h-full text-center relative z-10">
        <div className="max-w-lg mx-auto space-y-6">
          <h1 className="text-white text-4xl font-bold leading-tight drop-shadow-lg">
            {formData?.step1?.pageTitle || "Título da Proposta"}
          </h1>

          <p className="text-white/90 text-xl leading-relaxed drop-shadow-md">
            {formData?.step1?.pageSubtitle ||
              "Subtítulo da proposta aparecerá aqui com uma descrição mais detalhada"}
          </p>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {(formData?.step1?.services ?? []).length > 0 ? (
              ...
              <span className="bg-white/20 backdrop-blur-md text-white px-4 py-2 rounded-full text-sm border border-white/30 shadow-lg">
                Serviços aparecerão aqui
              </span>
            )}
          </div>
          ...
          <div className="pt-4">
            ...
            >
              {formData?.step1?.ctaButtonTitle || "Botão de Ação"}
            </button>
          </div>
        </div>
      </div>
