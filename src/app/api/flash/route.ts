...
export async function GET(request: Request) {
  try {
    ...
    // Count total Flash template projects (excluding archived)
    ...
    // Count sent Flash projects (excluding archived)
    ...
    // Fetch Flash template projects (excluding archived)
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Status do projeto Flash atualizado com sucesso",
      data: updatedProject[0],
    });
  } catch (error) {
    console.error("Error updating Flash project:", error);
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  try {
    ...
    if (updatedProjects.length === 0) {
      return NextResponse.json(
        { success: false, error: "Falha ao atualizar os projetos Flash" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `${updatedProjects.length} projeto(s) Flash atualizado(s) com sucesso`,
      data: updatedProjects,
      updatedCount: updatedProjects.length,
    });
  } catch (error) {
    console.error("Error updating Flash projects:", error);
    return NextResponse.json(
      { success: false, error: `Erro interno do servidor: ${error}` },
      { status: 500 }
    );
