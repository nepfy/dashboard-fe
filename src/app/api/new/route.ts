...
export async function GET(request: Request) {
  try {
    ...
    // Count total New template projects (excluding archived)
    ...
    // Fetch New template projects (excluding archived)
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Status do projeto New atualizado com sucesso",
...
export async function PATCH(request: Request) {
  try {
    ...
    if (updatedProjects.length === 0) {
      return NextResponse.json(
        { success: false, error: "Falha ao atualizar os projetos New" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `${updatedProjects.length} projeto(s) New atualizado(s) com sucesso`,
      data: updatedProjects,
      updatedCount: updatedProjects.length,
    });
  } catch (error) {
    console.error("Error updating New projects:", error);
    return NextResponse.json(
      { success: false, error: `Erro interno do servidor: ${error}` },
      { status: 500 }
    );
