...
export default function RowEditMenu({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      // Don't close if clicking inside modals or the menu itself
  ...
  }, [isOpen, onClose, showStatusPanel, showArchiveModal, showDuplicateModal]);
  ...
  return (
    <>
      <div
        ref={menuRef}
        className="e0 border border-white-neutral-light-300 rounded-[12px] absolute right-4 bg-white-neutral-light-100 z-10 min-w-[230px] sm:min-w-[280px]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className={`relative ${showStatusPanel ? "h-[390px]" : "h-full"}`}>
          {/* Main Menu */}
          <div
            className={`${
              showStatusPanel ? "opacity-0 pointer-events-none" : "opacity-100"
      ...
      >
        ...
        <p className="text-white-neutral-light-500 text-sm px-6 py-3 sm:p-6">
          Tem certeza que deseja duplicar este <br />
          tem? Uma cópia será criada <br /> imediatamente.
        </p>
