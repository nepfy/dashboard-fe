...
async function createFotografoAgent() {
  try {
    console.log(
      "🎯 Creating fotógrafo agent (with accent) for flash template..."
    );

    // Create base agent with accent
    const baseAgentId = "fotógrafo-base-agent";
    const flashAgentId = "fotógrafo-flash-agent";
    const primeAgentId = "fotógrafo-prime-agent";
    ...
    console.log("✅ Fotógrafo agents created successfully!");
    ...
    console.error("❌ Error creating fotógrafo agent:", error);
    throw error;
  }
}

// Run the script
createFotografoAgent()
  .then(() => {
