...
(function () {
  ...
  function renderPlans(containerId, plans) {
    ...
    // If no non-recommended template exists, use any pricing card and we'll handle the class manually
    if (!template) {
    ...
    visiblePlans.forEach((plan) => {
      ...
      // Handle is--center class - explicitly remove first to ensure clean state
      clone.classList.remove("is--center");
      if (isRecommended) {
        clone.classList.add("is--center");
      }

      // Handle badge - ensure it exists or create it
      let badge = clone.querySelector(".pricing_badge");
      if (isRecommended) {
        // If recommended but no badge exists, create it
        if (!badge) {
